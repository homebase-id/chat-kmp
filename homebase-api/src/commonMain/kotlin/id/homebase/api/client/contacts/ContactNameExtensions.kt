...
/**
 * Single source of truth for turning a contact's [ContactName] into the values the UIs render,
 * shared by the contact-book read path (`ContactBookEntry`) and the chat read path
 * (`DriveContactService` → `ContactUiModel`) so the two can no longer drift.
 *
