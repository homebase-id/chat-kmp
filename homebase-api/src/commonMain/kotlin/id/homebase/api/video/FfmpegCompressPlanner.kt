...
object FfmpegCompressPlanner {
     ...
     *   the same reason. Note: output is pinned to 8-bit SDR-decodable
    ...
    fun plan(
        ...
        val args = buildList {
            ...
            // -pix_fmt pins depth/subsampling but NOT colour metadata: ffmpeg copies the
            // source's primaries/transfer onto the output, so an HDR clip would come out
            // 8-bit yet still TAGGED BT.2020 PQ/HLG and be treated as HDR downstream.
            // Re-tag as BT.709 SDR whenever the source isn't positively known SDR
            // (fail closed, matching the bit-depth gate). These are bitstream tags, not
            // filters — no libzimg, nothing that can fail the encode.
            //
            // Ceiling: tagging is not tone-mapping. PQ-encoded pixels re-tagged BT.709
            // decode everywhere but look flat/dark. A real tone-map needs zscale+tonemap,
            // which the bundled ffmpeg builds do not include.
            if (probedIsHdr != false) {
