...
/**
 * The Enter chord end to end through a real composer field, on the JVM where `isDesktopOrWeb()` is
 * true. The attachment caption is the field that used to match `Key.Enter` alone.
 */
...
class ComposerEnterChordTest {
    ...
    @Test
    fun shiftEnterSendsByDefault() = runCaption(
        enterSendsMessage = false,
        press = { withKeyDown(Key.ShiftLeft) { pressKey(Key.Enter) } },
    ) {
        assertEquals(1, it.sends)
    }

    @Test
    fun bareEnterSendsWhenThePreferenceIsOn() = runCaption(
    ...
    @Test
    fun shiftEnterBreaksTheLineWhenThePreferenceIsOn() = runCaption(
        enterSendsMessage = true,
        press = { withKeyDown(Key.ShiftLeft) { pressKey(Key.Enter) } },
    ) {
        assertEquals(0, it.sends)
    }

    @Test
    fun ctrlEnterSendsInBothModes() {
        runCaption(
            enterSendsMessage = false,
            press = { withKeyDown(Key.CtrlLeft) { pressKey(Key.Enter) } },
        ) { assertEquals(1, it.sends) }
        runCaption(
            enterSendsMessage = true,
            press = { withKeyDown(Key.CtrlLeft) { pressKey(Key.Enter) } },
    ...
    /** The caption used to match Key.Enter alone, so macOS Return never reached it. */
    @Test
