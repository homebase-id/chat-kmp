...
fun MessageItem(
    ...
    val onMessageInfo = remember(message.id) { { onUiAction(ConversationListUiAction.ShowMessageInfo(message)) } }
    val onReply = remember(message.id) { { onUiAction(ConversationListUiAction.ReplyToMessage(message)) } }
    val onShare = remember(message.id) { { onUiAction(ConversationListUiAction.ShareMessage(message)) } }
    val onDelete = remember(message.id) { { onUiAction(ConversationListUiAction.DeleteMessage(message.id)) } }
    ...
    if (message.isCurrentUser(odinId)) {
        val onEdit = remember(message.id) { { onUiAction(ConversationListUiAction.EditMessage(messageId = message.id, versionTag = message.versionTag, ignoreDraft = false)) } }
